    ...
    <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
      ...
      <form action="/admin/auser" method="get">
        ...
        <div class="dataTables_length" id="DataTables_Table_0_length">
          <label>显示
            <select name="showCount" aria-controls="DataTables_Table_0" class="select">
              ...
              <option value="30" @if((isset($req['showCount']) && !empty($req['showCount'])) && $req['showCount'] == 30) selected @endif>30</option></select>条</label>
        ...
        <div class="text-c" style="float: right;">
            ...
            <input type="submit" class="btn btn-success radius" value="搜索" value="{{ $req['search'] or '' }}">
        </div>
      </form>
      ...
      <table class="table table-border table-bordered table-hover table-bg table-sort dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
        ...
        @foreach($auser as $k=>$v)
          <tr class="text-c odd" role="row">
            ...
            <td>{{ $v->created_at }}</td>
            <td class="td-manage">
              <a href="/admin/auser/{{ $v->id }}/edit" class="btn btn-warning btn btn-default btn-sm radius">修改</a>
              <form action="/admin/auser/{{ $v->id }}" method="post" style="display: inline-block;">
              	{{ csrf_field() }} 
              	{{ method_field('DELETE') }}
