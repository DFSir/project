...
<div class="infosbox">
    <div class="newsview">
        ...
        <div class="news_con">
            ...
            @endif
            <form action="/home/articles/{{ $article->aid }}" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                <div style="margin-top: 10px;">
                    ...
                    <span>
                        <select name="cid" id="" style="height: 30px;">
                            ...
                            ?>
                            <option value="{{ $v->cid }}" @if ($v->cpid == 0) disabled @endif @if ($article->cid == $v->cid) selected @endif>
                                @if ($v->cpid != 0) &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp @endif|------- {{ $v->cname }}
                            </option>
                            @endforeach
                        </select>
                    </span>
                ...
                <div style="margin-top: 10px;">
                    <span>文章标题：</span>
                    <span>
                        <input type="text" value="{{ $article->title }}" name="title" style="height: 30px;">
                    </span>
                </div>
                <div style="margin-top: 10px;">
                    <span>文章标签：</span>
                    <span>
                        ...
                        <label style="font-size: 14px;font-weight: normal;margin-right: 10px;">
                            <input type="checkbox" name="tag_id[]" value="{{$v->id}}" @if(in_array($v->id, $article->tags()->lists('id')->toArray())) checked @endif>{{$v->name}}</label>
                        @endforeach
                    </span>
                </div>
                <div style="margin-top: 10px;">
                    <span>文章作者：</span>
                    <span>
                        <input type="text" value="{{ $article->author }}" name="author" style="height: 30px;">
                    </span>
                </div>
                <div style="margin-top: 10px;">
                    <span>文章图片：</span>
                ...
                <div style="margin-top: 10px;margin-bottom: 30px;">
                    <span>
                        <input type="submit" value="修改文章" style="height: 50px;width: 100%;font-size: 25px;">
                    </span>
                </div>
